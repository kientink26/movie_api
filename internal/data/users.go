...
func (m UserModel) GetAll(permission string, name string, email string, filters Filters) ([]*User, Metadata, error) {
	query := fmt.Sprintf(`
SELECT count(*) OVER(), users.id, users.created_at, users.name, users.email, users.password_hash, users.activated, users.version
FROM users
...
AND (STRPOS(LOWER(name), LOWER($2)) > 0 OR $2 = '')
AND (STRPOS(LOWER(email), LOWER($3)) > 0 OR $3 = '')
ORDER BY users.%s %s, users.id ASC
LIMIT $4 OFFSET $5`, filters.sortColumn(), filters.sortDirection())
	args := []interface{}{permission, name, email, filters.limit(), filters.offset()}
